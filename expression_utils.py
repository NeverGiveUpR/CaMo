...
###############################################################################
def AND(x1, x2):
    return np.array([x1_i and x2_i for x1_i, x2_i in zip(x1, x2)])

def OR(x1, x2):
    return np.array([x1_i or x2_i for x1_i, x2_i in zip(x1, x2)])

def NOT(x):
    return np.array([not x_i for x_i in x])

def NOR(x1, x2):
    return np.array([not (x1_i or x2_i) for x1_i, x2_i in zip(x1, x2)])

def NAND(x1, x2):
    return np.array([not (x1_i and x2_i) for x1_i, x2_i in zip(x1, x2)])
...
def layer_construct_from_deep(root):
    layer = 0
    layer_node = {}
    def construct_(root, layer):
        nonlocal layer_node
        if layer in layer_node.keys():
            layer_node[layer].append(root)
        else:
            layer_node[layer] = [root]
        if root.left_child:
            construct_(root.left_child, layer+1)
        if root.right_child:
            construct_(root.right_child, layer+1)

    construct_(root, layer)
    return layer_node


def print_binary_tree(operators, root):
    def print_binary_tree_(root):
        print(root, operators.operator_list[int(str(root))])
        if root.left_child:
            print("left_child:")
            print_binary_tree_(root.left_child)
        if root.right_child:
            print("right_child:")
            print_binary_tree_(root.right_child)
        return

    print_binary_tree_(root)
...
class Expression(nn.Module):
    ...
    def forward(self, x):
        # eval: output the value(type:float) of expression(type:str)
        try:
...
def optimize_constants(expressions, X_constants, y_constants, inner_lr, inner_num_epochs, optimizer):
    ...
    if (optimizer=='lbfgs'):
        def closure():
            ...
            y = torch.nan_to_num(y, nan=100000)
